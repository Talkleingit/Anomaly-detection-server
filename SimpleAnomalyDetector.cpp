...
/**
 * @brief Function learns which columns in the time series are correlative and
 * saves its findings in the member cf
 *
 * @param ts
 */
void SimpleAnomalyDetector::learnNormal(const TimeSeries &ts)
{
  ...
  // intialize arrays of 2d points for each couple of correlated features. The
  // size of each point array == feature_size.
  ...
  // for all arrays of points and their linear regression line, calculate the
  // max deviation:
  for (int i = 0; i < NUM_OF_CORRELATED_FEATURES; i++)
  {
    /*
...
/**
 * @brief Searches for anomalies in a given time series object. If an anomaly is
 * found a report is saved in a vector of AnomalyReport
 *
 * @param ts
...
vector<AnomalyReport> SimpleAnomalyDetector::detect(const TimeSeries &ts)
{
  ...
  for (int i = 0; i < cf.size(); i++)
  { // iterate over all the elements in the cf member
    for (int j = 1; j <= size; j++)
    {
      ...
      {
        deviation = dev(*point, SimpleAnomalyDetector::cf[i].lin_reg); // calculate deviation from the linear regression line that was calculated in the learnNormal function
      }

      delete point; // free allocated Point.
      if (deviation > SimpleAnomalyDetector::cf[i].threshold)
      {
        // verifying if the deviation is bigger than the threshold found in the learnNormal function

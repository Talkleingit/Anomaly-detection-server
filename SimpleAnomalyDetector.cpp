...
/**
 * @brief Function learns which column in the time series are correlative and saves its findings in the member cf
 * 
 * @param ts 
 */
void SimpleAnomalyDetector::learnNormal(const TimeSeries& ts) {
	...
	for (int i =0; i < num_of_features; i++) {
		for (int j=0; j< num_of_features; j++) {
			if (i != j) {    //not the same feature     
				...
				if (info.corrlation >= 0.9) {   //verifying if the abs correlation of the two features is in fact correlative (over 0.9)
	...
	for (int i = 0; i < NUM_OF_CORRELATED_FEATURES; i++) {
		...
		for (int j = 0; j<feature_size; j++) {
			float deviation;
			deviation = dev(*(vec_of_point_arrays[i][j]), SimpleAnomalyDetector::cf[i].lin_reg);    //find deviation between a given point and the linear regression of 2 correlative features.
	...
	for (int i =0; i< cf.size(); i++) {
		if (SimpleAnomalyDetector::cf[i].corrlation >= 0.9) {
			for (int j =1; j<= size; j++) {
			     ...
			     const int feature2_index = ts.get_index_of_feature(SimpleAnomalyDetector::cf[i].feature2);    //find the index in which feature1 is placed in the row   
				 float x = row[feature1_index];
				 float y = row[feature2_index];
				 Point* p = new Point(x,y);    //create a point that the x value is from feature 1 and y value from feature2
				 float deviation  = dev(*p,SimpleAnomalyDetector::cf[i].lin_reg);    //calculate deviation from the linear regression line that was calculated in the learnNormal function    
